$(function(){
		...
		/*
		//模态中按钮水波纹样式切换
		$(".modal>.modal-footer").each(function(){
		...
		*/
		...
		//split.png图标点击跳转数据中心
		var aSplit = $(".main .content li.cont .cont-wrap.cont1 .pro-list-action");
		...
		$('.modal').modal({
			ready: function(modal, trigger) { 
				...
				//设备加高格式验证
				$(".high").blur(function(){
					if($(this).val() && !$(".high").val().match(/^[1-9]\d*(\.\d+)?$/)){
						$(this).css({
						...
						.next().css("color","red")
						.next().show();
					}else if($(".high").val().match(/^[1-9]\d*(\.\d+)?$/)){
						$(this).css({
			...
			complete: function(modal) { 
				//console.log($(modal[0]));     //$(modal[0])：当前模态的jq对象
