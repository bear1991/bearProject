...
var Lotterynumber = $('.select1 option:selected').text();

console.log(xinm);

$(function() {
	nametxt.attr('src', xinm[0]);
	phonetxt.html(phone[0]);
});

// 开始停止
function start() {
	Lotterynumber = $('.select1 option:selected').text();
	if(runing) {
		...
		if(pcount >= Lotterynumber) {
			stopTime = window.setInterval(function() {
				...
				if(runing) {
					...
					if(i == $('.select1 option:selected').text()) {
						console.log("抽奖结束");
